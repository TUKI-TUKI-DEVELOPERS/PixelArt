...
INSERT INTO prompt_shared_blocks (block_key, content) VALUES ('identidad_humano', $psb2$Usá cada imagen de referencia adjunta ÚNICAMENTE como guía de identidad (estructura ósea, rasgos faciales, edad y tono de piel) del personaje correspondiente — cada personaje es la persona real de su foto. NO copies, recortes ni pegues las fotos de referencia: RE-DIBUJÁ por completo cada rostro y cada cabeza desde cero como parte nativa de esta escena. Descartá el fondo, el encuadre, el ángulo de cabeza, la iluminación original y los accesorios incidentales de cada foto (aros, collares, cadenas, mechones sueltos, ropa) — nada de eso debe aparecer salvo que la escena lo pida explícitamente. El ángulo de la cabeza, la inclinación y la dirección de la mirada de cada personaje deben coincidir con su pose en la escena. Cada rostro debe re-iluminarse íntegramente con la luz de la escena y compartir el mismo grano, nitidez, profundidad de campo y temperatura de color que el resto de la escena — NO debe notarse ningún rostro fotográfico "pegado" sobre un cuerpo renderizado; las personas y la escena son una única fotografía tomada en ese mismo instante. Mantené fiel el parecido real de cada persona. La edad aparente de cada personaje es la edad real de su foto de referencia, sea niño, adolescente o adulto, sin importar lo que sugiera el contexto de la escena. El vestuario y los accesorios de cada personaje son EXCLUSIVAMENTE los descritos en esta escena (no lo que tenga puesto en su foto de referencia), excepto los lentes de armazón (no de sol), que sí se conservan si la persona los usa habitualmente.

El nivel de zoom o recorte de la foto de referencia (por ejemplo, un primer plano de rostro sin hombros visibles) NO indica el tamaño real de la cabeza de la persona — nunca repliques esa proporción de encuadre en la escena final. Las proporciones anatómicas de la cabeza respecto al cuerpo de cada personaje deben ser realistas y naturales para su edad real: la cabeza NO debe verse grande ni pequeña respecto a los hombros, el torso y las extremidades. Construí un cuerpo completo (torso, brazos, piernas) a escala anatómicamente correcta para cada cabeza, usando la escala corporal típica de una persona real de esa edad — NUNCA la escala aparente de la cara en la foto de referencia — como personas reales fotografiadas de cuerpo entero en su pose, evitando el efecto de "cabeza pegada sobre un cuerpo más chico".$psb2$)
  ON CONFLICT (block_key) DO UPDATE SET content = EXCLUDED.content, updated_at = now();

INSERT INTO prompt_shared_blocks (block_key, content) VALUES ('identidad_mascota', $psb3$Usá la imagen de referencia de la mascota para que sea el mismo animal real de esa foto — mantené su pelaje (color, patrón, manchas), color de ojos y rasgos distintivos reales, e integralo de forma natural y fotorrealista con la iluminación, el enfoque y el estilo del resto de la escena, como si hubiera sido fotografiado ahí mismo.

El nivel de zoom o recorte de la foto de referencia (por ejemplo, un primer plano de la cara de la mascota) NO indica el tamaño real de su cabeza — nunca repliques esa proporción de encuadre en la escena final. Construí un cuerpo completo (torso, patas, cola) a escala anatómicamente correcta y realista para esa especie y esa cabeza, como un animal real fotografiado de cuerpo entero en su pose, evitando el efecto de "cabeza pegada sobre un cuerpo más chico".$psb3$)
  ON CONFLICT (block_key) DO UPDATE SET content = EXCLUDED.content, updated_at = now();
